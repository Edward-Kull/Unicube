...
UniCube.prototype.onFlipEnd = function(fn) {
    let events = ["webkitTransitionEnd", "oTransitionEnd", "transitionend"];
...
UniCube.prototype._setContent = function() {
    let indexSide = Math.floor(-this.y / 90)
    ...
    if (this.horizontalFlip) {
        ...
        let content1 = cycleArray(this.content, indexSide);
        let content2 = cycleArray(this.content, nextIndex);
...
UniCube.prototype._createSides = function() {
    let sides = ["front", "right", "back", "left", "top", "bottom"];
...
UniCube.prototype._directionChanged = function(movedMouse) {
    let diffX = this.startX - movedMouse.x;
    let diffY = this.startY - movedMouse.y;
...
UniCube.prototype._getTouchDirection = function(movedMouse) {
    let diffX = mouse.start.x - movedMouse.x;
    let diffY = mouse.start.y - movedMouse.y;
...
UniCube.prototype._directionDisabled = function() {
    let index = Math.floor(-this.y / 90) || 0;
    let content = cycleArray(this.content, index);
